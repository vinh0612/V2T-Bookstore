...
<style>
    .detail-hero {
        background: linear-gradient(135deg, #0a3622 0%, #1a5c3a 50%, #0d4a2b 100%);
    }
    ...
    .detail-btn-cart:hover {
        ...
        box-shadow: 0 6px 20px rgba(10, 54, 34, 0.4);
    }
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 40px; align-items: center;">
            ...
            <div style="flex: 1; min-width: 280px; color: white;">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 24px;">
                    @php
                    // Lọc ra các review sạch để tính toán
                    $approvedReviews = $book->reviews->where('status', 'approved');
                    $realAvgRating = $approvedReviews->avg('rating');
                    @endphp
                    ...
                    <span style="display: inline-flex; align-items: center; gap: 4px; padding: 5px 14px; border-radius: 20px; font-size: 12px; font-weight: 700; background: rgba(245,158,11,0.2); color: #fbbf24;">
                        ★ {{ number_format($realAvgRating, 1) }} / 5.0
                    </span>
                    @endif
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        {{-- Đánh giá --}}
        <div>
            <h2 style="font-family: 'Playfair Display', serif; font-size: 1.5rem; font-weight: 700; color: #111827; padding-left: 16px; border-left: 4px solid #0a3622; margin-bottom: 20px;">
                Đánh giá từ độc giả ({{ $approvedReviews->count() }})
            </h2>
            ...
            <div style="display: flex; flex-direction: column; gap: 12px;">
                ...
                {{-- TRƯỜNG HỢP 1: BÌNH LUẬN ĐÃ DUYỆT (Sạch) -> Cho hiển thị bình thường --}}
                ...
                <div class="detail-review-card" style="display: flex; gap: 14px; align-items: flex-start;">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        ...
                        <div style="margin-top: 10px; background: #f9fafb; border-left: 3px solid #0a3622; padding: 10px; border-radius: 0 8px 8px 0; font-size: 0.8125rem;">
                            <div style="font-weight: 700; color: #0a3622; margin-bottom: 2px;">💬 Phản hồi từ V2T:</div>
                ...
                {{-- TRƯỜNG HỢP 2: BÌNH LUẬN BỊ AI BÁO CÁO (Shadowban) -> CHỈ HIỆN VỚI NGƯỜI ĐĂNG --}}
                @elseif($review->status === 'violated' && Auth::check() && Auth::id() === $review->user_id)
                <div class="detail-review-card" style="display: flex; gap: 14px; align-items: flex-start; opacity: 0.7; background: #fff5f5; border: 1px dashed #fca5a5;">
                    <div class="detail-avatar" style="background: #fee2e2; color: #b91c1c; border: 1px solid #fca5a5;">
                        {{ strtoupper(substr($review->user->name ?? 'U', 0, 1)) }}
                    </div>
                    <div style="flex: 1; min-width: 0;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                        ...
                        <p style="color: #dc2626; font-weight: 700; font-size: 0.8rem; margin: 4px 0 8px 0;">
                            ⚠️ Bình luận của bạn đã vi phạm tiêu chuẩn cộng đồng và bị ẩn.
                        </p>
                        <p style="font-size: 0.875rem; color: #9ca3af; line-height: 1.6; text-decoration: line-through; font-style: italic;">
                            {{ $review->comment }}
                        </p>
                    </div>
                </div>
