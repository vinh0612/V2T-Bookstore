                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-4 text-gray-400">
                            ...
                            <button onclick="submitDeleteVoucher(this)" data-id="{{ $voucher->id }}" class="hover:text-red-600 transition p-1" title="Xóa mã">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-1.8c0-.632-.425-1.18-.074-1.18h-4.2c-.65 0-1.074.549-1.074 1.18v1.8m4.5 0H9" />
                                </svg>
                            </button>
                        </div>
                    </td>
...
<div id="voucherModal" class="fixed inset-0 z-50 flex items-center justify-center hidden">
    ...
    <div class="bg-white rounded-xl shadow-2xl z-10 w-full max-w-lg overflow-hidden border border-gray-100">
        ...
        <form action="{{ route('admin.vouchers.store') }}" method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-600 mb-1">Đơn tối thiểu áp dụng (đ)</label>
                    <input type="number" name="min_order_value" value="0" min="0" placeholder="Ví dụ: 100000" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1e3e36]">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-600 mb-1">Ngày bắt đầu</label>
                    <input type="date" name="start_date" min="{{ date('Y-m-d') }}" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1e3e36]">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-600 mb-1">Ngày hết hạn</label>
                    <input type="date" name="end_date" min="{{ date('Y-m-d') }}" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1e3e36]">
                </div>
            </div>
            <div>
    ...
    <div class="bg-white rounded-xl shadow-2xl z-10 w-full max-w-lg overflow-hidden border border-gray-100">
        ...
        <form id="editVoucherForm" method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-600 mb-1">Đơn tối thiểu áp dụng (đ)</label>
                    <input type="number" id="edit_min_order_value" name="min_order_value" min="0" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1e3e36]">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-600 mb-1">Ngày bắt đầu</label>
                    <input type="date" id="edit_start_date" name="start_date" min="{{ date('Y-m-d') }}" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1e3e36]">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wide text-gray-600 mb-1">Ngày hết hạn</label>
                    <input type="date" id="edit_end_date" name="end_date" min="{{ date('Y-m-d') }}" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1e3e36]">
                </div>
            </div>
            <div>
...
<script>
    ...
    function openEditModal(button) {
        ...
        document.getElementById('edit_min_order_value').value = voucher.min_order_value || 0;
